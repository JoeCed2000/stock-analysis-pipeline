...
## 2026-08-10 — Feedback visibility restored + working tree drained

**Trigger:** Ced reported that the page listing posted client comments had disappeared, and that `#admin` did not show them either.

**Two unrelated causes, no data loss** (16 entries across 8 buckets in `analyses/feedback_*/index.json`):

1. **Public `#feedback` — intended.** The global submission history was removed so a public page cannot enumerate every visitor's feedback; submitters now see only their own reference id. `FeedbackPage.publicHistory.test.cjs` locks this in.
2. **`#admin` — regression.** `c10d148` correctly put `/api/admin/feedback` behind `_require_auth`, but the static bundle sends no key, so the viewer 403'd. Fixed: `#admin` now prompts for `CED_CONTROL_KEY` (password field, `sessionStorage`, never `localStorage`), sends `X-API-Key`, and `getFeedbackAttachmentUrl()` appends `api_key` because an `<a href>` cannot carry a header. One shared helper covers both call sites.

**Two defects found in uncommitted code, fixed before committing:**

- `App.jsx handleViewReport` — the popup-blocker fix was right in direction but `window.open(…, 'noopener')` returns `null`, so the "allow pop-ups" warning fired on **every successful open**; and navigating straight to the PDF URL dropped the `422`/`pdf_blocked` branch and the `MAX_PDF_POLL_ATTEMPTS` cap recorded as delivered on 2026-05-31 — a blocked PDF showed the client raw JSON. The tab is now claimed synchronously inside the gesture (opener severed by hand) and filled by the restored poll/422 logic.
- **Test pollution** — `transcript_finder`'s new 24h cache globs the live `analyses/` root, so a real NVDA dossier written the same morning outranked the monkeypatched source. The suite passed or failed depending on whether an analysis had run that day. `tests/conftest.py` now neutralizes the cache by autouse fixture. `analyses/` itself is deliberately not redirected: `DeepDiveRequest` validates `output_dir` against that exact root.

**Orphan work recovered:** the `t_677a97e5` security closeout (documented here but never committed) and the `t_323b071e` repair (written in the `t_bd8c1098` worktree, never merged back).

**Repo hygiene:** working tree 48 dirty entries → 5. Untracked `.worktrees/` (81 MB registered worktree), untracked two stale `.bak` sources (1397 lines), collapsed three duplicate `*.bak` rules in `.gitignore` into one pattern that actually matches `.bak_<date>`/`.bak2`.

**Verification:** full suite **19 failed → 16 failed, 1389 → 1392 passed, zero new regressions** (failure lists diffed). The 16 remaining are strictly pre-existing and untouched. 9/9 frontend guards, `npm run build` OK, prod bundle byte-identical to `dist/`. Backend restarted (`systemctl --user restart stock-pipeline.service`, fresh PID); prod returns 200 on the page and bundle, and 403 on `/api/admin/feedback`, `/api/feedback`, `/api/feedback-file` without a key.

**Docs corrected:** `DEPLOY.md` described a Vercel + Render deployment that was never the production setup — rewritten to the real topology (systemd `--user` units + Cloudflare Tunnel, backend serving `frontend/dist/`). `CLAUDE.md` listed four backend modules that no longer exist at those paths and a `Feedback.jsx` that never existed; also recorded that `SA_SKIP_CODEX` is read by **no code** (only a stale comment) — the live switch is `SA_COMPANY_OVERVIEW_SKIP_CODEX`.

**Still open:** the two `t_56a3086a` evidence artifacts remain unstageable (see the 2026-07-15 entry below).
...
|---|---|---|---|
| 2026-08-10 | **Feedback visibility restored + working tree drained + docs corrected** | Ced reported the client-comments page had vanished from both `#feedback` and `#admin`. Two unrelated causes, no data loss (16 entries, 8 buckets). Public `#feedback`: global history removal was **intended** (a public page must not enumerate every visitor's feedback) — kept, guarded. `#admin`: **regression** — `c10d148` correctly protected `/api/admin/feedback` behind `_require_auth`, but the static bundle sent no key, so the viewer 403'd; `#admin` now prompts for `CED_CONTROL_KEY` (password field, `sessionStorage`, never `localStorage`), sends `X-API-Key`, and `getFeedbackAttachmentUrl()` appends `api_key` since an `<a href>` cannot carry a header. Two defects found in uncommitted code and fixed before committing: `window.open(…, 'noopener')` returns `null`, so the "allow pop-ups" toast fired on every **successful** open; and navigating straight to the PDF URL dropped the `422`/`pdf_blocked` branch plus the `MAX_PDF_POLL_ATTEMPTS` cap delivered 2026-05-31, showing clients raw JSON — tab now claimed synchronously inside the gesture, poll/422 logic restored. Test pollution fixed: `transcript_finder`'s new 24h cache globbed the live `analyses/` root, so results depended on whether an analysis had run that day (`tests/conftest.py` autouse fixture). Orphan work recovered: `t_677a97e5` security closeout and the `t_323b071e` repair stranded in worktree `t_bd8c1098`. Hygiene: 48 dirty entries → 5; untracked `.worktrees/` (81 MB) and two stale `.bak` sources; collapsed three duplicate `*.bak` gitignore rules into one that actually matches `.bak_<date>`/`.bak2`. Docs: `DEPLOY.md` described a Vercel + Render deployment that was never production — rewritten to the real topology; `CLAUDE.md` listed four backend modules absent at those paths and a `Feedback.jsx` that never existed; `SA_SKIP_CODEX` recorded as dead (no code reads it — live switch is `SA_COMPANY_OVERVIEW_SKIP_CODEX`). Verification: full suite **19 failed → 16 failed, 1389 → 1392 passed, zero new regressions** (failure lists diffed; the 16 are strictly pre-existing and untouched); 9/9 frontend guards; `npm run build` OK; prod bundle byte-identical to `dist/`; backend restarted via `systemctl --user restart stock-pipeline.service` (fresh PID); prod page + bundle `200`, and `403` on `/api/admin/feedback`, `/api/feedback`, `/api/feedback-file` without a key. Commits: `29a83db`, `f91c716`, `bbdc82c`, `1adac13`. | ✅ DONE |
